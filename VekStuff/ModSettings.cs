...
namespace TabletopTweaks.Config {
    class ModSettings {
        ...
        public static void LoadLocalization() {
            ...
            var resourcePath = $"Findus.Localization.{fileName}"; ;
            ...
            if (File.Exists(localizationPath)) {
                ...
                using (JsonReader jsonReader = new JsonTextReader(streamReader)) {
                    try {
                        ...
                        Findus.Main.logger.Error("Failed to localization. Settings will be rebuilt.");
                        try { File.Copy(localizationPath, ModEntry.Path + $"{Path.DirectorySeparatorChar}BROKEN_{fileName}", true); } catch { Findus.Main.logger.Error("Failed to archive broken localization."); }
                    }
                }
            } else {
        ...
        public static void SaveLocalization(string fileName, MultiLocalizationPack localizaiton) {
            ...
            Findus.Main.logger.Log($"Localization: {ModLocalizationPack.Strings.Count}");
        }
        private static void LoadSettings<T>(string fileName, ref T setting) where T : IUpdatableSettings {
            try
            {
                JsonSerializer serializer = JsonSerializer.Create(SerializerSettings);
                var assembly = Assembly.GetExecutingAssembly();
                var resourcePath = $"Findus.Config.{fileName}";
                ...
                if (File.Exists(userPath))
                {
                    ...
                    using (JsonReader jsonReader = new JsonTextReader(streamReader))
                    {
                        ...
                        {
                            Findus.Main.logger.Error("Failed to load user settings. Settings will be rebuilt.");
                            try { File.Copy(userPath, userConfigFolder + $"{Path.DirectorySeparatorChar}BROKEN_{fileName}", true); } catch { Findus.Main.logger.Error("Failed to archive broken settings."); }
                        }
                    }
                }
            ...
            {
                Findus.Main.logger.Log(e.ToString());
            }
        }
