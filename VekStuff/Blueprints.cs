        ...
        public BlueprintGuid GetGUID(string name) {
            ...
            if (!NewBlueprints.TryGetValue(name, out Id)) {
                ...
                if (!AutoGenerated.TryGetValue(name, out Id)) {
                    ...
                    Findus.Main.logger.Log($"Generated new GUID: {name} - {Id}");
                } else {
                    Findus.Main.logger.Log($"WARNING: GUID: {name} - {Id} is autogenerated");
                }
#endif
            }
            if (Id == null) { Findus.Main.logger.Log($"ERROR: GUID for {name} not found"); }
            UsedGUIDs[name] = Id;
